...
void test_multi_head(int batch, int heads, int seq_k)
{
    ...
    // CPU reference — process each (batch, head) independently
...
int main()
{
    // Régression head_dim=128
    test_single_head<128>(BQ);    // seq_k=64
    test_single_head<128>(128);   // seq_k=128
    ...
    // Multi-head
    test_multi_head<128>(1, 4, 128);   // batch=1, 4 heads
    test_multi_head<128>(2, 4, 128);   // batch=2, 4 heads
}